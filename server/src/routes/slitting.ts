import { Router, Request, Response } from 'express';
import { executeQuery } from '../config/database';

const router = Router();

/**
 * 안전하게 문자열로 변환하는 헬퍼 함수
 */
const toString = (val: any): string => {
  if (val === null || val === undefined) return '';
  if (typeof val === 'string') return val;
  if (typeof val === 'number') return String(val);
  if (typeof val === 'boolean') return String(val);
  if (typeof val === 'object') {
    // 객체인 경우 CODE_NAME이나 NAME을 우선적으로 사용
    if (val.CODE_NAME) return String(val.CODE_NAME);
    if (val.NAME) return String(val.NAME);
    if (val.CODE) return String(val.CODE);
    if (val.name) return String(val.name);
    if (val.label) return String(val.label);
    if (val.value) return String(val.value);
    // 그 외에는 빈 문자열 반환
    return '';
  }
  return String(val);
};

/**
 * 근무조 목록 조회
 * DB FUNCTION: SF_GET_SHIFT('01','PR09')
 */
router.get('/shifts', async (req: Request, res: Response) => {
  try {
    console.log('Fetching shifts from DB function SF_GET_SHIFT...');
    
    const result = await executeQuery(
      "SELECT SF_GET_SHIFT('01','PR09') AS SHIFT FROM DUAL"
    );
    
    console.log('Function result:', result.rows);
    
    let shifts: any[] = [];
    
    if (result.rows && result.rows.length > 0) {
      const functionResult = (result.rows[0] as any).SHIFT;
      console.log('Function result value:', functionResult, 'Type:', typeof functionResult);
      
      // 결과가 null이거나 undefined인 경우
      if (!functionResult) {
        console.warn('Function returned null or undefined');
        return res.json({
          status: 'success',
          data: []
        });
      }

      // 결과가 문자열인 경우 파싱
      if (typeof functionResult === 'string') {
        // JSON 형식인지 확인
        if (functionResult.trim().startsWith('[') || functionResult.trim().startsWith('{')) {
          try {
            const parsed = JSON.parse(functionResult);
            if (Array.isArray(parsed)) {
              shifts = parsed.map((item: any, index: number) => ({
                value: toString(item.CODE || item.code || item.value),
                label: toString(item.CODE_NAME || item.NAME || item.name || item.label || item.CODE || item.code || item.value),
                id: index + 1
              }));
            } else {
              shifts = [{
                value: toString(parsed.CODE || parsed.code || parsed.value),
                label: toString(parsed.CODE_NAME || parsed.NAME || parsed.name || parsed.label || parsed.CODE || parsed.code || parsed.value),
                id: 1
              }];
            }
          } catch (parseError) {
            // JSON 파싱 실패 시 콤마로 구분된 문자열로 처리
            if (functionResult.includes(',')) {
              shifts = functionResult.split(',').map((item: string, index: number) => ({
                value: item.trim(),
                label: item.trim(),
                id: index + 1
              }));
            } else {
              shifts = [{
                value: functionResult.trim(),
                label: functionResult.trim(),
                id: 1
              }];
            }
          }
        } else if (functionResult.includes(',')) {
          // 콤마로 구분된 문자열인 경우
          shifts = functionResult.split(',').map((item: string, index: number) => ({
            value: item.trim(),
            label: item.trim(),
            id: index + 1
          }));
        } else {
          // 단일 값인 경우
          shifts = [{
            value: functionResult.trim(),
            label: functionResult.trim(),
            id: 1
          }];
        }
      } else if (Array.isArray(functionResult)) {
        // 배열인 경우 - CODE를 value로, CODE_NAME을 label로 사용
        shifts = functionResult.map((item: any, index: number) => ({
          value: toString(item.CODE || item.code || item.value),
          label: toString(item.CODE_NAME || item.NAME || item.name || item.label || item.CODE || item.code || item.value),
          id: index + 1
        }));
      } else if (typeof functionResult === 'object') {
        // 객체인 경우 - CODE를 value로, CODE_NAME을 label로 사용
        shifts = [{
          value: toString(functionResult.CODE || functionResult.code || functionResult.value),
          label: toString(functionResult.CODE_NAME || functionResult.NAME || functionResult.name || functionResult.label || functionResult.CODE || functionResult.code || functionResult.value),
          id: 1
        }];
      } else {
        // 기타 타입
        shifts = [{
          value: String(functionResult),
          label: String(functionResult),
          id: 1
        }];
      }
    }
    
    console.log('Parsed shifts:', shifts);
    
    res.json({
      status: 'success',
      data: shifts
    });
  } catch (error: any) {
    console.error('Error fetching shifts:', error);
    res.status(500).json({
      status: 'error',
      message: 'Failed to fetch shifts',
      error: error.message
    });
  }
});

/**
 * 설비 목록 조회
 * DB FUNCTION: SF_GET_WC_LIST('01','PR02')
 */
router.get('/equipment', async (req: Request, res: Response) => {
  try {
    console.log('Fetching equipment from DB function SF_GET_WC_LIST...');
    
    const result = await executeQuery(
      "SELECT SF_GET_WC_LIST('01','PR02') AS EQUIPMENT FROM DUAL"
    );
    
    console.log('Function result:', result.rows);
    
    let equipment: any[] = [];
    
    if (result.rows && result.rows.length > 0) {
      const functionResult = (result.rows[0] as any).EQUIPMENT;
      console.log('Function result value:', functionResult, 'Type:', typeof functionResult);
      
      // 결과가 null이거나 undefined인 경우
      if (!functionResult) {
        console.warn('Function returned null or undefined');
        return res.json({
          status: 'success',
          data: []
        });
      }

      // 결과가 문자열인 경우 파싱
      if (typeof functionResult === 'string') {
        // JSON 형식인지 확인
        if (functionResult.trim().startsWith('[') || functionResult.trim().startsWith('{')) {
          try {
            const parsed = JSON.parse(functionResult);
            if (Array.isArray(parsed)) {
              equipment = parsed.map((item: any, index: number) => ({
                value: toString(item.CODE || item.code || item.value),
                label: toString(item.CODE_NAME || item.NAME || item.name || item.label || item.CODE || item.code || item.value),
                id: index + 1
              }));
            } else {
              equipment = [{
                value: toString(parsed.CODE || parsed.code || parsed.value),
                label: toString(parsed.CODE_NAME || parsed.NAME || parsed.name || parsed.label || parsed.CODE || parsed.code || parsed.value),
                id: 1
              }];
            }
          } catch (parseError) {
            // JSON 파싱 실패 시 콤마로 구분된 문자열로 처리
            if (functionResult.includes(',')) {
              equipment = functionResult.split(',').map((item: string, index: number) => ({
                value: item.trim(),
                label: item.trim(),
                id: index + 1
              }));
            } else {
              equipment = [{
                value: functionResult.trim(),
                label: functionResult.trim(),
                id: 1
              }];
            }
          }
        } else if (functionResult.includes(',')) {
          // 콤마로 구분된 문자열인 경우
          equipment = functionResult.split(',').map((item: string, index: number) => ({
            value: item.trim(),
            label: item.trim(),
            id: index + 1
          }));
        } else {
          // 단일 값인 경우
          equipment = [{
            value: functionResult.trim(),
            label: functionResult.trim(),
            id: 1
          }];
        }
      } else if (Array.isArray(functionResult)) {
        // 배열인 경우 - CODE를 value로, CODE_NAME을 label로 사용
        equipment = functionResult.map((item: any, index: number) => ({
          value: toString(item.CODE || item.code || item.value),
          label: toString(item.CODE_NAME || item.NAME || item.name || item.label || item.CODE || item.code || item.value),
          id: index + 1
        }));
      } else if (typeof functionResult === 'object') {
        // 객체인 경우 - CODE를 value로, CODE_NAME을 label로 사용
        equipment = [{
          value: toString(functionResult.CODE || functionResult.code || functionResult.value),
          label: toString(functionResult.CODE_NAME || functionResult.NAME || functionResult.name || functionResult.label || functionResult.CODE || functionResult.code || functionResult.value),
          id: 1
        }];
      } else {
        // 기타 타입
        equipment = [{
          value: String(functionResult),
          label: String(functionResult),
          id: 1
        }];
      }
    }
    
    console.log('Parsed equipment:', equipment);
    
    res.json({
      status: 'success',
      data: equipment
    });
  } catch (error: any) {
    console.error('Error fetching equipment:', error);
    res.status(500).json({
      status: 'error',
      message: 'Failed to fetch equipment',
      error: error.message
    });
  }
});

export default router;
